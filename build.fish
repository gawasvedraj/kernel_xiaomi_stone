...
set -a args KBUILD_BUILD_VERSION=1
set -a args KBUILD_BUILD_TIMESTAMP="Thu Jan 1 00:00:00 UTC 1970"
...
cp $OUTD/Image $AK3/ || exit
cp $OUTD/dtb.img $AK3/dtb || exit
cp $OUTD/dtbo.img $AK3/dtbo || exit

if test $argv[1]
	cd $AK3 && zip -x "*.zip" -r9 $argv[1]-(date -u +"%Y%m%d-%H%M").zip * && cd -
end
end # pack
